 ...
 * 
 */
public class DrivingModelCumulative extends Task {
	// --- Task Code ---//
	...
	private final double  cumulativeFactor= 1; 
	...
	public void update(double time) {
		try {
			if (time - simulationStartTime <= simulationDurarion) {
				...
				getModel().getFatigue().setCumulativeParameter(simulationNumber*(simulationDurarion/60)* cumulativeFactor);
	...
	public static Image getImage(final String name) {
		URL url = DrivingModelCumulative.class.getResource("images/" + name);
	...
	public Result analyze(Task[] tasks, boolean output) {
		...
		try {
			...
			for (Task taskCast : tasks) {
				DrivingModelCumulative task = (DrivingModelCumulative) taskCast;
