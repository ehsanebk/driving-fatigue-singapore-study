...
public class PVT_modelB_Accumulative extends Task {
	...
	public PVT_modelB_Accumulative() {
		super();
	...
	public void update(double time) {
		...
		// Starting a new Session
			...
			if (sessionNumber < timesOfPVT.length) {
				addEvent(new Event(getModel().getTime() + 240*60, "task", "update") { // after 4 hours: 240 min
					@Override
					public void action() {
	...
	public Result analyze(Task[] tasks, boolean output) {
		...
		try {
			...
			for (Task taskCast : tasks) {
				PVT_modelB_Accumulative task = (PVT_modelB_Accumulative) taskCast;
				for (int i = 0; i < numberOfSessions; i++) {
